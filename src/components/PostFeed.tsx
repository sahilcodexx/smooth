...
export function PostFeed({ initialPosts }: PostFeedProps) {
  ...
  return (
    <TooltipProvider>
      <main className="max-w-[37em]" style={{ margin: '6rem auto', padding: '2rem 1.25rem', fontFamily: 'var(--font-choice, var(--font-geist))' }}>
          ...
          {posts.length > 0 ? (
            posts.map((post) => (
              <a 
                ...
                style={{ textDecoration: 'none', display: 'block', padding: '0.75rem 0', borderBottom: '1px solid var(--color-border)', transition: 'border-color 0.2s' }} 
                className="post-item"
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <motion.div 
                    initial={false}
                    animate={{ width: isDeleteMode ? 28 : 0, opacity: isDeleteMode ? 1 : 0 }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '1rem', flex: 1, minWidth: 0 }}>
                    <h2 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 400, color: 'var(--color-foreground)', transition: 'color 0.2s' }} className="post-title-link">
                      {truncateTitle(post.title)}
                    </h2>
                    <span style={{ fontSize: '0.8rem', color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-geist-mono)', flexShrink: 0 }}>
            ...
            <div style={{ textAlign: 'center', padding: '4rem 1rem', border: '1px dashed var(--color-border)', borderRadius: '0.75rem' }}>
              <p style={{ color: 'var(--color-muted-foreground)', marginBottom: '1.5rem', fontSize: '0.95rem' }}>No posts yet. Start sharing your thoughts.</p>
              <a href="/create?new=true" style={{ textDecoration: 'none', padding: '0.6rem 1.25rem', backgroundColor: 'var(--color-foreground)', color: 'var(--color-background)', borderRadius: '9999px', fontSize: '0.85rem', fontWeight: 500 }}>
      ...
      <div className="dark fixed bottom-8 left-1/2 -translate-x-1/2 z-50 pointer-events-auto">
        <AnimatePresence mode="wait">
          {!isDeleteMode ? (
            ...
            >
              <Dock className={`bg-[#09090b]/96 border border-zinc-800/80 rounded-full px-3 py-2 flex items-center gap-1.5 ring-1 ring-white/10 mx-auto ${isDark ? 'shadow-[0_12px_40px_rgba(0,0,0,0.6)]' : 'shadow-none'}`} iconSize={40} iconMagnification={58}>
                {/* Create Post */}
                <DockIcon 
            ...
            <motion.div
              ...
              transition={{ duration: 0.2 }}
            >
              <Dock className={`bg-[#09090b]/96 border border-zinc-800/80 ring-1 ring-white/10 mx-auto ${isDark ? 'shadow-[0_12px_40px_rgba(0,0,0,0.6)]' : 'shadow-none'}`} iconSize={40} iconMagnification={58}>
